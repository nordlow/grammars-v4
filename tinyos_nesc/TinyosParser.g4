...
options { tokenVocab=TinyosLexer;
	 superClass=MultiChannelBaseParser;
}


compilationUnit
: (includeDeclarationModule* componentDeclaration)? (includeDeclarationConfiguration* componentDeclaration EOF)
;
...
call_stat
: CALL common_name call_condition_block SCOL?
;
...
| TASK VOID? common_name task_condition_block task_stat_block? ( stat )* (TASK VOID? common_name task_stat_block)?
;
...
| STATIC VOID? common_name static_condition_block static_stat_block? ( stat )* (STATIC VOID? common_name static_stat_block)?
;
...
| VOID? common_name other_condition_block other_stat_block? ( stat )* ( VOID? common_name other_stat_block)?
;
...
while_stat
: WHILE OPAR expr CPAR while_stat_block
;
...
singleDoubleArray
:OBRACE arrayElement* CBRACE
;

arrayElement
: atom ','?
| (OBRACE atom ',' atom  CBRACE) ','?
;

chars
:(OPAR | CPAR | INC | DEC | FORWARDARROW | BACKARROW | COLONCOLON | AT | COMMA | MULT | GT | LT | DOT | ASSIGN | BITAND | OBRACK | CBRACK )
;

chars_no_comma
:(OPAR | CPAR | INC | DEC | FORWARDARROW | BACKARROW | COLONCOLON | AT | MULT | GT | LT | DOT | ASSIGN | BITAND | OBRACK | CBRACK )
;

reservedwords
: (VOID | RETURN | AS | POST | ATOMIC | ERROR | ABSTRACT | NEW | CALL |BREAK)
;
...
: (componentsDefinition | componentsWiring | platformDefinition)*
;
platformDefinition
...
: atom
| name_with_char
| NEW atom | NEW atom '('? atom? ')'?
| NEW atom AS atom | NEW? atom '('? atom? ')'? AS atom
;
